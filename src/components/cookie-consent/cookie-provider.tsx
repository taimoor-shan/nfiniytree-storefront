...
export function CookieConsentProvider({
  ...
  // Initialize state from localStorage — once per mount. The boundary
  // rebuilds `config` on every navigation (usePathname), so keying this
  // effect on it would re-run the whole init on each locale switch:
  // localStorage re-read, a new state object, and re-renders of every
  // context consumer.
  ...
  React.useEffect(() => {
    ...
    // config.consentVersion and config.traceability are constant strings and
    // flags set once by the boundary; only the object identity changes.
  ...
  const saveAndTrack = React.useCallback(
    async (
      ...
      // Track consent if traceability is enabled. Fire-and-forget: the audit
      // post (tracker.ts retries internally and queues failed records) must
      // not block banner dismissal or the settings dialog closing.
      ...
      if (traceability?.enabled) {
        void (async () => {
          try {
            ...
            // Tracking is best-effort; consent is already persisted locally.
          }
        })()
      }
    },
