...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  return getCmsPageMetadata({
    ...
    countryCode,
    titleKey: "footer.returnsRefunds",
    descriptionKey: "metadata.returnsDescription",
  })
}
        ...
        {page?.content ? (
          ...
          <>
            <h1 className="text-2xl font-semibold text-ink">
              {await translate("footer.returnsRefunds", locale)}
            </h1>
            <div className="mt-6 space-y-4 text-body">
              <p>{await translate("policy.pageBeingUpdated", locale)}</p>
              <p>
                {await translate("policy.forReturnRequests", locale)}{" "}
                <a
